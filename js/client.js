...
(function ($) {
	...
	var user = localStorage.getItem("user") ? JSON.parse(localStorage.getItem("user")) : {name : "Guest",id : "guest",registered: false} 
	...
	var User = (this.User = Backbone.Model.extend({
		defaults: {
			name: user.name,
			userid: user.id,
			registered: user.userid == "guest" ? false:true,
			named: true,
			avatar: user.avatar,
			settings: {},
			status: "",
				...
				battles.forEach((battle) => {
					if (!battle) {
						...
						self.rooms[battle.trim()].receive("|j|"+ (user ? user.name : "Guest"))
							...
							self.rooms[battle.trim()].receive("|notstarted|")
							self.rooms[battle.trim()].receive("|slotupdate|2")
		...
		switch (data[1]) {
			...
			if (roomid) {
				if (this.rooms[roomid]) {
					//console.log(roomid + " --@@@@-- " + JSON.stringify(this.rooms[roomid]));
